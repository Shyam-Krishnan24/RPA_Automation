...
function renderBenchmarksTable(filter: string = '') {
  ...
  filtered.forEach(bench => {
    ...
    row.innerHTML = `
      ...
      <td>
        <button class="table-action-btn" title="More Actions">
...
function setupEventListeners() {
  ...
  // More models dropdown popup display
  const modelDropdownBtn = document.getElementById('model-more-dropdown-btn');
  if (modelDropdownBtn) {
    modelDropdownBtn.addEventListener('click', () => {
      alert("Additional models (e.g. Llama 3, Qwen 2, Cohere) are available on the Enterprise tier.");
    });
  }
...
function handleRunBenchmark() {
  ...
  // In the layout, we also have the secondary "Or paste your text here" box.
  // If no file was selected but there is content in the paste area, we can use that as text benchmark name
  if (currentTab === 'file' && !selectedFile) {
    ...
    if (pasteVal) {
      ...
      alert("Please drop a file or paste text in the editor before running.");
      return;
    }
  }
  ...
  setTimeout(() => {
    ...
    const newBenchmark: Benchmark = {
      id: (benchmarks.length + 1).toString(),
      name: name,
      type: type,
      models: activeModels,
